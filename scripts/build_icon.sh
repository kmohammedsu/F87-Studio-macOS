#!/bin/zsh
set -euo pipefail

project_dir="${0:A:h:h}"
iconset="$project_dir/Resources/Assets.xcassets/AppIcon.appiconset"
master="$project_dir/.build/AppIcon-master.png"
mkdir -p "$iconset"

swiftc -module-cache-path "$project_dir/.build/module-cache" \
  "$project_dir/scripts/IconRenderer.swift" -o "$project_dir/.build/icon-renderer"
"$project_dir/.build/icon-renderer" "$master"

for entry in "16 icon_16x16.png" "32 icon_16x16@2x.png" "32 icon_32x32.png" "64 icon_32x32@2x.png" "128 icon_128x128.png" "256 icon_128x128@2x.png" "256 icon_256x256.png" "512 icon_256x256@2x.png" "512 icon_512x512.png" "1024 icon_512x512@2x.png"; do
  size="${entry%% *}"
  name="${entry#* }"
  sips -z "$size" "$size" "$master" --out "$iconset/$name" >/dev/null
done
